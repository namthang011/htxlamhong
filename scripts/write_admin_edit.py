...
new_script = """<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
            ...
            fetch("/api/public/services?size=100").then(res => res.json()).then(data => {
                ...
                if(data.content) {
                    ...
                    data.content.forEach(s => {
                        html += `<tr><td>${s.id}</td><td><img src="${s.imageUrl}" height="40"></td><td>${s.name}</td><td>${s.price || "Liên hệ"}</td><td><span class="badge bg-${s.status=="ACTIVE"?"success":"secondary"}">${s.status}</span></td>
                        <td>
                            <button class="btn btn-sm btn-primary" onclick="editService(${s.id})">Sửa</button>
        ...
        function saveService() {
            ...
            fetch(url, { method: method, headers: { "Authorization": jwtToken, "Content-Type": "application/json" }, body: JSON.stringify(data) }).then(res => { if(res.ok) { serviceModal.hide(); loadServices(); } else alert("Lỗi khi lưu!"); });
        }
            ...
            fetch("/api/public/posts?size=100").then(res => res.json()).then(data => {
                ...
                if(data.content) {
                    ...
                    data.content.forEach(p => {
                        ...
                        <td>
                            <button class="btn btn-sm btn-primary" onclick="editPost(${p.id})">Sửa</button>
        ...
        function savePost() {
            ...
            fetch(url, { method: method, headers: { "Authorization": jwtToken, "Content-Type": "application/json" }, body: JSON.stringify(data) }).then(res => { if(res.ok) { postModal.hide(); loadPosts(); } else alert("Lỗi khi lưu!"); });
        }
