...
new_script = """<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
    <script>
            ...
            fetch("/api/public/services").then(res => res.json()).then(data => {
                ...
                if(data.content) {
                    data.content.forEach(s => {
                        html += `<tr><td>${s.id}</td><td><img src="${s.imageUrl}" height="40"></td><td>${s.name}</td><td>${s.price || "Liên h?"}</td><td><span class="badge bg-${s.status=="ACTIVE"?"success":"secondary"}">${s.status}</span></td><td><button class="btn btn-sm btn-danger" onclick="deleteService(${s.id})">Xóa</button></td></tr>`;
                    });
                }
        ...
        function saveService() {
            ...
            fetch("/api/admin/services", { method: "POST", headers: { "Authorization": jwtToken, "Content-Type": "application/json" }, body: JSON.stringify(data) }).then(res => { if(res.ok) { serviceModal.hide(); loadServices(); } else alert("L?i khi luu!"); });
        }
        ...
        function savePost() {
            ...
            fetch("/api/admin/posts", { method: "POST", headers: { "Authorization": jwtToken, "Content-Type": "application/json" }, body: JSON.stringify(data) }).then(res => { if(res.ok) { postModal.hide(); loadPosts(); } else alert("L?i khi luu!"); });
        }
